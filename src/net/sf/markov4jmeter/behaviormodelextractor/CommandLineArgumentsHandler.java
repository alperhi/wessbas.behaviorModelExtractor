 ...
 * Extractor. Each option is initiated by a leading hyphen; an overview is given
 ...
 * "examples/sessions.dat".</td>
 * 
 * <tr>
 * <td><code> output </code></td>
 * <td><code> o      </code></td>
 * <td>Output directory for the resulting files, e.g., "output/".</td>
 * 
 * <tr>
 ...
 * <td><code> l         </code></td>
 * <td>(Optional) OS-specific line-break type for CSV output files (0 = Windows,
 * 1 = Unix, 2 = MacOS); the default value is 0 (Windows).</td>
 * 
 * <tr>
 * <td><code> template </code></td>
 * <td><code> t        </code></td>
 * <td>(Optional) input CSV-file which includes a template matrix, e.g.,
 * "examples/template.csv".</td>
 * 
 * <tr>
 * <td><code> mapping </code></td>
 * <td><code> m       </code></td>
 * <td>(Optional) use case mapping file, e.g., "examples/mapping.csv".</td>
 * 
 * <tr>
 * <td><code> clustering </code></td>
 * <td><code> c          </code></td>
 * <td>(Optional) Clustering method to be applied to the Behavior Models which
 * have been extracted from session traces; available values are "simple" and
 * "kmeans". If no clustering is specified, the resulting output files contain
 * the trace-related matrices/graphs, as they are computed before any clustering
 * method is applied to the extracted Behavior Models.</td>
 * 
 * <tr>
 * <td><code> number of clusters </code></td>
 * <td><code> n          </code></td>
 * <td>(Optional) Specifies the number of clusters that are generated with
 * kmeans clustering.</td>
 * </table>
 * 
 * <p>
 * Examples:
 ...
 * <code>-i "./examples/sessions.dat" -o "./output/"</code> </blockquote>
 * denotes a minimum start configuration for the Behavior Model Extractor, which
 * uses "./examples/sessions.dat" as input file and "./output/" as output
 * directory.</li>
 * 
 * <li>The options sequence <blockquote>
 * <code>-i "./examples/sessions.dat" -o "./output/" -l 2</code> </blockquote>
 ...
 *     -t "./examples/template.csv"</code> </blockquote> has the same effect as
 * the second one, but it additionally passes a template file for being filled
 * with values.</li>
 * </ul>
 * 
...
public class CommandLineArgumentsHandler {
	 ...
	 * command-line.
	 * 
	 * @return a valid <code>String</code> which represents a file path, or
	...
	public static void init(final String[] args) throws ParseException,
		...
		CommandLineArgumentsHandler.inputFile = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.INPUT_FILE);

		CommandLineArgumentsHandler.outputDirectory = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.OUTPUT_DIRECTORY);

		CommandLineArgumentsHandler.templateFile = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.TEMPLATE_FILE);

		CommandLineArgumentsHandler.useCaseMappingFile = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.USE_CASE_MAPPING_FILE);

		CommandLineArgumentsHandler.lineBreakType = CommandLineArgumentsHandler
				.readOptionValueAsInt(commandLine,
						CommandLineArgumentsHandler.LINE_BREAK_TYPE);

		CommandLineArgumentsHandler.clusteringMethod = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.CLUSTERING_METHOD);

		CommandLineArgumentsHandler.numberOfClustersMin = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
						CommandLineArgumentsHandler.NUMBER_OF_CLUSTERS_MIN);

		CommandLineArgumentsHandler.numberOfClustersMax = CommandLineArgumentsHandler
				.readOptionValueAsString(commandLine,
	 ...
	 *             value.
	 */
	private static int readOptionValueAsInt(final CommandLine commandLine,
			final Option option) throws NullPointerException,
