 ...
 * 
 * <p>
 * Absolute Behavior Models are equipped with "absolute" probabilities and a set
 * of time ranges for each transition. Absolute probability values denote the
 * number of transition-occurrences within a session; they serve as basis for
 * the calculation of actual probabilities. The time ranges serve as basis for
 * the calculation of think times.
 * 
...
public class SessionToABMTransformer {
	 ...
	 * detected.
	 */
	private final static String WARNING_NEGATIVE_TIME_RANGE = "negative time range detected in transition from state \"%s\" to "
	 ...
	 * 
	 * <p>
	 * A list of use cases might be passed additionally, for associated vertices
	 * being added to each Behavior Model by default; such use cases might
	 * originate from a template matrix, which possibly provides additional uses
	 * case that are not included to monitored data. if <code>null</code> is
	 * passed, the transformation starts with an empty set of vertices.
	 * 
	 * @param sessions
	 *            sessions to be transformed to Behavior Models.
	 * @param defaultUseCases
	 *            list of use cases, which indicates vertices to be created by
	 *            default.
	 ...
	 *            <code>true</code> if and only if registered initial use cases
	 *            shall collected individually for each model.
	 * 
	 * @return the resulting Behavior Models.
	 */
	public BehaviorModelAbsolute[] transformSessionsToBehaviorModels(
			final List<Session> sessions, final List<UseCase> defaultUseCases,
			final IdGenerator useCaseIdGenerator,
			final boolean clearRegisteredInitialUseCasesInEachIteration) {

		final ArrayList<BehaviorModelAbsolute> behaviorModelsAbsolute = new ArrayList<BehaviorModelAbsolute>();

		final InitialUseCaseHandler initialUseCaseHandler = new InitialUseCaseHandler(
				useCaseIdGenerator);
		...
		for (final Session session : sessions) {
			...
			final BehaviorModelAbsolute behaviorModelAbsolute = this
					.transformSessionToBehaviorModel(session, defaultUseCases,
							initialUseCaseHandler);

			behaviorModelsAbsolute.add(behaviorModelAbsolute);
		}

		this.determineUniqueInitialState(behaviorModelsAbsolute,
				defaultUseCases, initialUseCaseHandler);
	 ...
	 * <p>
	 * A list of use cases might be passed additionally, for associated vertices
	 * being added by default; such use cases might originate from a template
	 * matrix, which possibly provides additional uses case that are not
	 * included to monitored data. if <code>null</code> is passed, the
	 * transformation starts with an empty set of vertices.
	 * 
	 * @param session
	 *            session to be transformed to a Behavior Model.
	 * @param defaultUseCases
	 *            list of use cases, which indicate vertices to be created by
	 *            default.
	 * @param initialUseCaseHandler
	 *            handler for counting occurrences of initial use cases.
	 * 
	 * @return the resulting Behavior Model.
	 */
	private BehaviorModelAbsolute transformSessionToBehaviorModel(
			final Session session, final List<UseCase> defaultUseCases,
			final InitialUseCaseHandler initialUseCaseHandler) {

		final BehaviorFactory factory = BehaviorFactory.eINSTANCE;

		final BehaviorModelAbsolute behaviorModelAbsolute = factory
				.createBehaviorModelAbsolute();
		...
		final LinkedList<Vertex> vertices = this.transformSessionToVertices(
				session, defaultUseCases, initialUseCaseHandler);
	 ...
	 *            handler for counting occurrences of initial use cases.
	 * 
	 * @return the resulting vertices, including their outgoing transitions.
	 */
	private LinkedList<Vertex> transformSessionToVertices(
		...
		for (final ObservedUseCaseExecution dstUCExecution : session
				.getObservedUseCaseExecutions()) {
			...
			if (srcUCExecution == null) {
				...
				initialUseCaseHandler.registerInitialState(dstVertex
						.getUseCase());
				...
				final Vertex srcVertex = this.findVertex(srcUCExecution,
						vertices);

				Transition transition = this.findTransition(srcVertex,
						dstVertex);
				...
				if (timeDistance < 0 && timeDistance2 >= 0) {
					...
					final String message = String
							.format(SessionToABMTransformer.WARNING_NEGATIVE_TIME_RANGE,
	 ...
	 */
	private Transition installTransition(final Vertex sourceVertex,
	 ...
	 * 
	 * @param ouce
	 *            instance which is associated with the vertex to be searched
	 *            for.
	 * @param vertices
	 ...
	 *         exist.
	 */
	private Vertex findVertex(final ObservedUseCaseExecution ouce,
	 ...
	 *         exist.
	 */
	private Vertex findVertexByUseCaseId(final String useCaseId,
	 ...
	 * 
	 * @return a transition between the given vertices, if one exists; otherwise
	 *         <code>null</code> will be returned.
	 */
	private Transition findTransition(final Vertex srcVertex,
	 ...
	 *            Behavior Models with absolute transition values; these
	 *            Behavior Models build on the given
	 *            <code>defaultUseCases</code> list.
	 * @param defaultUseCases
	 *            list of use cases, which indicates the vertices of the given
	 *            Behavior Models.
	 * @param initialUseCaseHandler
	 *            instance for registering initial use cases and generating a
	 *            generic initial use case, if necessary; in that case, the
	 *            generic use case including corresponding transitions will be
	 *            added as unique initial use case to each Behavior Model.
	 * 
	 * @return a unique use case, which is one of the given default use cases;
	 *         if multiple initial use cases exist in that list, a generic use
	 *         case will be created and returned.
	 */
	private UseCase determineUniqueInitialState(
		...
		final UseCase initialUseCase = initialUseCaseHandler
				.getUniqueInitialUseCase();
		...
		if (!defaultUseCases.contains(initialUseCase)) {
			...
			for (final BehaviorModelAbsolute behaviorModelAbsolute : behaviorModelsAbsolute) {

				final Vertex sourceVertex = this.createVertex(initialUseCase);

				final List<Vertex> vertices = behaviorModelAbsolute
						.getVertices();
				...
				for (final UseCase useCase : initialUseCaseHandler
						.getRegisteredInitialUseCases()) {

					final Vertex targetVertex = this.findVertexByUseCaseId(
							useCase.getId(), vertices);

					final Transition transition = this.installTransition(
